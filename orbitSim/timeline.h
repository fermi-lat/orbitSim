...
BOOST_FUSION_ADAPT_STRUCT( rockprofile_pair,
                           ( double, rocktime )( double, rockangle ) )

BOOST_FUSION_ADAPT_STRUCT( rocking_profile,
                           ( std::string, rockstart )( double, rockstart_met )(
                               double,
                               rockdefault )( std::vector<rockprofile_pair>,
                                              pairs ) )

BOOST_FUSION_ADAPT_STRUCT(
    opt_evt_fields,
    ( std::string, prop_ID )( std::string, target_name )( double, offset )(
        double,
        RA )( double, DEC )( std::string, PI )( std::string, comment )(
        int,
        week )( int, SSN )( double, duration )( int, slew )( int, saa )(
        double,
        dupRA )( double, dupDEC )( rocking_profile, profile ) )

BOOST_FUSION_ADAPT_STRUCT(
    timeline_initial,
    ( int, week )( std::string, timeline_name )( std::string, create_time )(
        std::string,
        creator )( double, RA )( double, DEC )( rocking_profile,
                                                profile )( timeline_event,
                                                           event )
    /* (std::vector<std::string>, tako_db) */
    /* (std::vector<std::string>, sc_ephem) */
    /* (std::vector<std::string>, saa) */
...
BOOST_FUSION_ADAPT_STRUCT(
    timeline_event,
    ( std::string, timestamp )( std::string, event_name )(
        std::string,
        event_type )( std::string, obs_number )( opt_evt_fields, additional ) )

BOOST_FUSION_ADAPT_STRUCT( command,
                           ( std::string, time )( std::string,
                                                  type )( std::string, order )
                           //(std::vector<std::string>,  params)
                           ( std::string, params ) )


BOOST_FUSION_ADAPT_STRUCT( timeline_header,
                           ( std::string, filename )
                           /* (std::string, creation_time) */
                           /* (std::string, mission_id) */
                           /* (std::string, originator) */
...
BOOST_FUSION_ADAPT_STRUCT( timeline_wrapper,
                           ( timeline_header, header )( timeline_initial, init )
                           //(std::vector<command>, commands)
                           ( std::vector<timeline_event>, events ) )
//]
...
struct timeline_grammar
    : qi::grammar<Iterator, timeline_wrapper(), ascii::space_type> {
  timeline_grammar()
      : timeline_grammar::base_type( timeline ) {
    using ascii::char_;
    using ascii::digit;
    using ascii::string;
    ...
    timestamp %= repeat( 4 )[digit] >> char_( "/" ) >> repeat( 3 )[digit]
                 >> repeat( 3 )[char_( ":" ) >> qi::repeat( 2 )[digit]];

    one_liner %= lexeme[+( char_ - qi::eol )];

    rocktime_angle %=
        lit( "//" ) >> qi::omit[digit >> digit] >> double_ >> double_;

    tl_rocking_profile %= lit( "//" ) >> lit( "Rocking Profile:" )
                          >> lit( "//" ) >> lit( "ROCKSTART" ) >> "="
                          >> timestamp >> '(' >> double_ >> ')' >> lit( "//" )
                          >> lit( "ROCKDEFAULT" ) >> "=" >> double_
                          >> lit( "//" ) >> lit( "ROCKTIME" )
                          >> lit( "ROCKANGLE" ) >> repeat( 17 )[rocktime_angle];

    RA %= lit( "//" ) >> lit( "RA" ) >> "=" >> qi::double_ >> -lit( "deg" );


    DEC %= lit( "//" ) >> qi::omit[qi::no_case["DEC"]] >> "=" >> qi::double_
           >> -lit( "deg" );


    tl_opt_evt_fields =
        ( lit( "//" ) >> lit( "prop_ID" ) >> "="
          >> one_liner )[at_c<0>( _val ) = _1]
        ^ ( lit( "//" ) >> lit( "target_name" ) >> "="
            >> one_liner )[at_c<1>( _val ) = _1]
        ^ ( lit( "//" ) >> lit( "offset" ) >> "=" >> qi::double_
            >> lit( "deg" ) )[at_c<2>( _val ) = _1]
        ^ RA[at_c<3>( _val ) = _1] ^ DEC[at_c<4>( _val ) = _1]
        ^ ( lit( "//" ) >> lit( "PI" ) >> "="
            >> one_liner )[at_c<5>( _val ) = _1]
        ^ ( lit( "//" ) >> lit( "comment" ) >> "="
            >> one_liner[at_c<6>( _val ) = _1] )
        ^ ( lit( "//" ) >> lit( "week" ) >> "="
            >> qi::int_[at_c<7>( _val ) = _1] )
        ^ ( lit( "//" ) >> lit( "SSN" ) >> "="
            >> qi::int_[at_c<8>( _val ) = _1] )
        ^ ( lit( "//" ) >> lit( "duration" ) >> "="
            >> qi::double_[at_c<9>( _val ) = _1] >> lit( "ksec" ) )
        ^ ( lit( "//" ) >> lit( "slew" ) >> "="
            >> qi::int_[at_c<10>( _val ) = _1] >> lit( "sec" ) )
        ^ ( lit( "//" ) >> lit( "saa" ) >> "="
            >> qi::int_[at_c<11>( _val ) = _1] >> lit( "sec" ) )
        ...
        // some reason in Tako Timelines. So For ease I've just crated
        // additional fields to hold them, the dupRA and dupDEC fields.
        ^ RA[at_c<12>( _val ) = _1] ^ DEC[at_c<13>( _val ) = _1]
    ...
    event %= lit( "//" ) >> timestamp
             ...
             >> -( lit( "obs_number" ) >> "="
                   >> lexeme[string( "Global" ) | +( digit | char_( "-" ) )] )
             >> -tl_opt_evt_fields;


    tl_initial %=
        lit( "//" ) >> "Mission Week:" >> int_ >> "//" >> "Timeline Name:"
        >> file_name >> lit( "Created:" ) >> timestamp >> "//" >> "Created with"
        >> one_liner >> "//" >> "Initial pointing" >> RA >> DEC >> "//"
        >> "Initial survey-related flight parameter settings"
        >> tl_rocking_profile >> -event >> "//" >> "// Input files:"
        ...
        /* >> "//" */
        /* >> "//" >> "Previous ARR Threshold was" >> *qi::alpha [at_c<8>(_val)
           = _1] */
    ...
    command %= timestamp >> ( string( "CMD" ) | string( "ACT" ) )
               >> lexeme[+alnum] >> -command_param >> ";";

    header %= file_name >> "," >> *( char_ - ";" ) >> ";";
    ...
    /*   /1* >> "," >> file_name               // Reference Timeline Filename
     * *1/ */
    /*   >> "," >> lexeme[+(char_ - ";")]  // Comment */
    /*   >> ";" */
    /*   ; */

    generic_comment %= !( event | tl_initial ) >> lit( "//" ) >> one_liner;

    ignored %= divider | command | generic_comment;

    timeline =
        *ignored >> -header[at_c<0>( _val ) = _1] >> *ignored
        >> -tl_initial[at_c<1>( _val ) = _1] >> *ignored
        >> *( event[phoenix::push_back( at_c<2>( _val ), _1 )] | ignored )
        >> *ignored >> qi::eoi;
  }
  ...
  // Sub-parsers
  typedef qi::rule<Iterator, std::vector<std::string>(), ascii::space_type>
                                                               str_vec;
