...
orbitSimApp::run()
{
  ...
  if ((initf.ELT_OFF_STOP != -99999.0 || initf.ELT_OFF_START != -99999.0))
  {
    if (initf.ELT_OFF_STOP <= initf.ELT_OFF_START)
    {
      ...
      osf.warn(1) << "These parameter are not acceptable, reverting to "
  ...
  if ((initf.ELT_OFF_STOP != -99999.0 && initf.ELT_OFF_START != -99999.0))
  {
    osf.info(1) << "EARTH Limb Tracing is disabled from " << initf.ELT_OFF_START
                << " to " << initf.ELT_OFF_STOP << "\n";
  }
  ...
  else if (match_str((const char*)Input.c_str(), "CONSOLE") == 1)
  {
    ...
    else
    {
      throw std::runtime_error(
        "\nERROR: Unknown Timeline type {expecting either TAKO, or ASFLOWN or "
        "SINGLE}\n\n");
    }
  ...
  osf.info(2) << __FILE__ << ":" << __LINE__
              ...
              << std::endl;
  osf.info(2) << __FILE__ << ":" << __LINE__ << " - stop MJD=" << initf.stop_MJD
              << " endTm=" << endTm << std::endl;
