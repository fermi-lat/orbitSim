/** \file main.cxx
  ...
  John Vernaleo (historical), FSSC
  Alex Reustle (current), FSSC
  */
...
void orbitSimApp::run() {
  ...
  if ( ( initf.ELT_OFF_STOP != -99999.0 || initf.ELT_OFF_START != -99999.0 ) ) {
    if ( initf.ELT_OFF_STOP <= initf.ELT_OFF_START ) {
      osf.warn( 1 )
          << "Earth LIMB Tracing OFF stop time is smaller than start time\n";
      osf.warn( 1 )
          << "These parameter are not acceptable, reverting to "
  ...
  if ( ( initf.ELT_OFF_STOP != -99999.0 && initf.ELT_OFF_START != -99999.0 ) ) {
    osf.info( 1 ) << "EARTH Limb Tracing is disabled from "
                  << initf.ELT_OFF_START << " to " << initf.ELT_OFF_STOP
                  << "\n";
  }
  ...
  if ( match_str( (const char*)Input.c_str(), "FILE" ) == 1 ) {
    ...
    if ( ( match_str( (const char*)Tlt.c_str(), "TAKO" ) == 1 )
         || ( match_str( (const char*)Tlt.c_str(), "ASFLOWN" ) == 1 ) ) {
      ...
      // Do the same even if it is single command, should be parsed the same way
      // TEST THIS FUNCTIONALITY IT MAY NOT WORK
      initf.TLname = Tml;
      /* initf.timeline.populate(initf.TLname); */
    } else {
      throw std::runtime_error( "\nERROR: Unknown Timeline type {expecting "
                                "either TAKO, or ASFLOWN or "
                                "SINGLE}\n\n" );
    }
    ...
    if ( !( ( match_str( (const char*)EpF.c_str(), "XYZLL_EPH" ) == 1 )
      ...
      throw std::runtime_error(
          "\nERROR: Unknown Ephemeris Function {expecting either xyzll_eph, or "
          "yyyy_eph or tlederive}\n\n" );
    } else {
      initf.EPHfunc = EpF;
    }
    pars.Prompt( "Units" );
    ...
    throw std::runtime_error( "\nERROR: Unknown Input type {expecting either "
                              "'file' or 'console'}\n\n" );
  }

  pars.Save();

  osf.info( 1 ) << "Earth Avoidance Angle: " << initf.EAA << " degrees"
                << std::endl;
  ...
  if ( stat == 0 ) {
    ...
    oBuf << "\n############################################################"
            ...
            "parenthesis allowed values where applicable\n\n";
    ...
    oBuf << "Timeline type          " << initf.TLtype
         << ",  (MUST be TAKO, ASFLOWN, SINGLE)" << std::endl;
    oBuf << "Ephemeris file       " << initf.EPHname << std::endl;
    oBuf << "Ephemeris func       " << initf.EPHfunc
         << ", (yyyy_eph, xyzll_eph, tlederive)" << std::endl;
  ...
  if ( ( initf.start_MJD - stmjd ) - initf.Resolution > 1.0E-6 ) {
    osf.info( 2 ) << "Initial start time=" << initf.start_MJD
                  << ", corrected time=" << stmjd << std::endl;
  ...
  if ( ( initf.stop_MJD - enmjd ) - initf.Resolution > 1.0E-6 ) {
    osf.info( 2 ) << "Initial stop time=" << initf.stop_MJD
                  << ", corrected time=" << enmjd << std::endl;
  ...
  osf.info( 2 ) << "Opening Ephemeris file " << initf.EPHname
                << " for reading\n";
  if ( ( ephF = fopen( initf.EPHname.c_str(), "r" ) ) == NULL ) {
    std::string fname( initf.EPHname );
    throw std::runtime_error( "Could not open Ephemeris file:\n" + fname );
  }

  initf.Resolution     = initf.Resolution / minInDay;
  EphemData* ephemeris = NULL;

  osf.info( 2 ) << "Populating Ephemeris structure by calling " << initf.EPHfunc
                << " function.\n";

  if ( match_str( initf.EPHfunc.c_str(), "YYYY_EPH" ) == 1 ) {
    ephemeris = yyyy_eph(
        ephF, initf.start_MJD, initf.stop_MJD, initf.Units, initf.Resolution );
  ...
  if ( ephemeris == NULL ) {
    throw std::runtime_error(
        "\nPossibly something went wrong while reading/generating "
        "ephemeris.\nThe Ephemeris structure is still \"NULL\"\n\n" );
  }

  osf.info( 3 ) << "From Ephem file, start mjd = " << ephemeris->MJD[0]
                << std::endl;
  ...
  if ( match_str( initf.TLtype.c_str(), "TAKO" ) == 1 ) {
    ...
    // throw std::runtime_error("\nERROR: ASFLOWN mode is still in developement.
  ...
  if ( Oat == NULL ) {
    throw std::runtime_error(
        "\nPossibly something went wrong while calculating the spacecraft "
        "attitude.\nThe Attitude structure is still \"NULL\"\n\n" );
  }

  // Get template file using facilities
  std::string orbitsimroot =
      facilities::commonUtilities::getDataPath( "orbitSim" );
  std::string ifname( "ft2.tpl" );
  std::string sfname =
      facilities::commonUtilities::joinPath( orbitsimroot, ifname );
  ...
  osf.info( 2 ) << __FILE__ << ":" << __LINE__
                << " - start MJD=" << initf.start_MJD << " startTm=" << startTm
                << std::endl;
  osf.info( 2 ) << __FILE__ << ":" << __LINE__
                << " - stop MJD=" << initf.stop_MJD << " endTm=" << endTm
                << std::endl;

  metstart = do_mjd2met( initf.start_MJD );
  metstop  = do_mjd2met( initf.stop_MJD );

  TypedImage<int>* tableP =
      IFileSvc::instance().editImageInt( initf.OutFile, "Primary" );
  ...
  for ( Table::Iterator itor = table->begin(); itor != table->end(); ++itor ) {
    ...
    // Careful interpolating at the array boundaries (don't assume extra
    // elements)
    ...
    /* Have to add a minus sign in order to match the rocking angle in real
     * spacecraft FT2 file. */
    ...
    if ( ( Stop - Start ) != (long int)( initf.Resolution * 86400.0 ) ) {
      osf.warn( 1 ) << "WARNING ===> At k=" << k << ", Interval is "
                    << ( Stop - Start ) << ", while resolution is "
                    << (long int)( initf.Resolution * 86400.0 ) << "\n";
    }

    if ( pStart != 0 ) {
      if ( ( Start - pStart ) != (long int)( initf.Resolution * 86400.0 ) ) {
        osf.warn( 1 ) << "WARNING ===> At k=" << k
                      << ", Interval (from Start time " << Start << ") is "
                      << ( Start - pStart ) << ", while resolution is "
                      << (long int)( initf.Resolution * 86400.0 ) << "\n";
      }
    }
    ...
    if ( pStop != 0 ) {
      if ( ( Stop - pStop ) != (long int)( initf.Resolution * 86400.0 ) ) {
        osf.warn( 1 ) << "WARNING ===> At k=" << k
                      << ", Interval (from Stop time " << Stop << ") is "
                      << ( Stop - pStop ) << ", while resolution is "
                      << (long int)( initf.Resolution * 86400.0 ) << "\n";
      }
    }
